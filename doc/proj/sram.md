...
| RTL           | CLKRST_CONNECTED           | Done        | Modules containing submodules checked: `axi_sram.sv` and `axi_to_detailed_mem.sv`. Modules without clocks and resets are confirmed to be purely combinational: `stream_join` and `stream_mux`.
