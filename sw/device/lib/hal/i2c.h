...
// Enable I2C block in target mode
void i2c_enable_target_mode(i2c_t i2c);

// Return the data in the controller's RX FIFO
uint8_t i2c_rdata_byte(i2c_t i2c);

// Return the acquire data in the ACQ FIFO
i2c_acqdata i2c_read_acqdata(i2c_t i2c);

// Write data in TX FIFO
void i2c_write_tx_data(i2c_t i2c, uint8_t data);

// Set I2C target's address and mask pairs
void i2c_set_target_id(i2c_t i2c, uint8_t addr0, uint8_t mask0, uint8_t addr1, uint8_t mask1);

// Clear cmd_complete interrupt state
void i2c_clear_cmd_complete(i2c_t i2c);

// Read the intr_state register to check the interrupt states generated byt the I2C transfer
i2c_intr i2c_read_intr_state(i2c_t i2c);

// Read status register to check the status of FIFOs during or at the end of the I2C transfer
i2c_status i2c_read_status(i2c_t i2c);

// Drains the entry that contains the start signal from the ACQ FIFO
bool i2c_target_drain_start_sig_acq_data(i2c_t i2c);

// Drains the entry that contains the end signal from the ACQ FIFO
