...
void ethernet_mac_address_set(ethernet_t ethernet, uint64_t address)
{
    ...
    DEV_WRITE64(ethernet + ETHERNET_MACHI_REG, reg | htons((uint16_t)(address & ETHERNET_MACHI_MACADDR_MASK)));
}
...
void ethernet_rx_promiscuous_enable(ethernet_t ethernet) {
    DEV_WRITE64(ethernet + ETHERNET_MACHI_REG, DEV_READ64(ethernet + ETHERNET_MACHI_REG) | ETHERNET_MACHI_ALLPKTS_MASK);
}
