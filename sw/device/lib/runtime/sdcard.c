...
bool collected_data(spi_host_t spi, uint8_t *buf, uint32_t len, uart_t uart);
bool read_cid_csd(spi_host_t spi, uint8_t cmd, uint8_t *buf, uint32_t len, uart_t uart);
void read_card_data(spi_host_t spi, uint8_t data[], uint32_t len);
void wait_idle(spi_host_t spi);
void nonblocking_cycles(spi_host_t spi, uint32_t cycles, bool csaat);
void nonblocking_write(spi_host_t spi, const uint8_t data[], uint32_t len);
