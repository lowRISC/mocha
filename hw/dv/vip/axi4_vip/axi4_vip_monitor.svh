...
class axi4_vip_monitor extends uvm_monitor;
  ...
  // Analysis Ports - Updated to specific transaction type for better type safety
  ...
  // Internal Storage
  // Protected to allow for extension/inheritance while shielding from external components
  protected axi4_vip_item aw_pending_q[$];
  protected axi4_vip_item w_pending_q[$];

  // ID-indexed queues to pair Request with Response
  protected axi4_vip_item write_q_by_id [bit [`AXI4_MAX_ID_WIDTH-1:0]] [$];
  protected axi4_vip_item read_q_by_id  [bit [`AXI4_MAX_ID_WIDTH-1:0]] [$];
  ...
  // Protected internal logic methods
  ...
  // Merges address-phase control information into a data-phase item.
  extern protected function void merge_tx(axi4_vip_item req, axi4_vip_item data);
    ...
    fork : isolation_fork
      begin
        fork
          ...
          // Monitor Threads: Run in parallel
          collect_aw_channel();
          collect_w_channel();
          collect_b_channel();
          collect_ar_channel();
...
task axi4_vip_monitor::collect_aw_channel();
  // Using explicit casting to MAX widths to prevent truncation/overflow on large shifts
  bit [    `AXI4_MAX_ID_WIDTH-1:0] id_mask;
  bit [  `AXI4_MAX_ADDR_WIDTH-1:0] addr_mask;
  bit [  `AXI4_MAX_USER_WIDTH-1:0] user_mask;
  bit [`AXI4_MAX_REGION_WIDTH-1:0] region_mask;
  bit [   `AXI4_MAX_QOS_WIDTH-1:0] qos_mask;

  id_mask     = (`AXI4_MAX_ID_WIDTH'(1) << m_cfg.m_id_width) - 1;
  addr_mask   = (`AXI4_MAX_ADDR_WIDTH'(1) << m_cfg.m_addr_width) - 1;
  user_mask   = (`AXI4_MAX_USER_WIDTH'(1) << m_cfg.m_user_width) - 1;
  region_mask = (`AXI4_MAX_REGION_WIDTH'(1) << m_cfg.m_region_width) - 1;
  qos_mask    = (`AXI4_MAX_QOS_WIDTH'(1) << m_cfg.m_qos_width) - 1;
    ...
    if (vif.monitor_cb.wvalid && vif.monitor_cb.wready) begin
      ...
      if (vif.monitor_cb.wlast) begin
        if (aw_pending_q.size() > 0) begin
          ...
          merge_tx(aw_tr, w_burst);
          write_q_by_id[aw_tr.awid].push_back(w_burst);
        end else begin
...
task axi4_vip_monitor::collect_r_channel();
  ...
  id_mask   = (`AXI4_MAX_ID_WIDTH'(1) << m_cfg.m_id_width) - 1;
  data_mask = (`AXI4_MAX_DATA_WIDTH'(1) << m_cfg.m_data_width) - 1;
  user_mask = (`AXI4_MAX_USER_WIDTH'(1) << m_cfg.m_user_width) - 1;
  ...
  forever begin
    ...
    if (vif.monitor_cb.rvalid && vif.monitor_cb.rready) begin
      ...
      if (read_q_by_id.exists(id) && read_q_by_id[id].size() > 0) begin
        axi4_vip_item tr = read_q_by_id[id][0];  // Peek at original request
        tr.rid = id;
