...
function void axi4_vip_monitor::stop_processes();
  ...
  foreach (p_list[i]) begin
    if (p_list[i] != null) p_list[i].kill();
  end
...
task axi4_vip_monitor::collect_aw_channel();
  forever begin
    ...
    if(vif.monitor_cb.awvalid && vif.monitor_cb.awready) begin
      ...
      `uvm_info(get_full_name(), $sformatf("AW transaction collected: %s", tr.sprint()), UVM_MEDIUM)
  ...
  forever begin
    ...
    if(vif.monitor_cb.wvalid && vif.monitor_cb.wready) begin
      ...
      if (vif.monitor_cb.wlast) begin
        ...
        `uvm_info(get_full_name(), $sformatf("W transaction collected: %s", current_burst.sprint()), UVM_MEDIUM)
  ...
  forever begin
    ...
    if(vif.monitor_cb.bvalid && vif.monitor_cb.bready) begin
      ...
      if(write_q_by_id.exists(id) && write_q_by_id[id].size() > 0) begin
        ...
        `uvm_info(get_full_name(), $sformatf("FULL AXI4 write transaction collected: %s", tr.sprint()), UVM_LOW)
...
task axi4_vip_monitor::collect_ar_channel();
  forever begin
    ...
    if(vif.monitor_cb.arvalid && vif.monitor_cb.arready) begin
      ...
      `uvm_info(get_full_name(), $sformatf("AR transaction collected: %s", tr.sprint()), UVM_MEDIUM)
    ...
    if(vif.monitor_cb.rvalid && vif.monitor_cb.rready) begin
      ...
      if(read_q_by_id.exists(id) && read_q_by_id[id].size() > 0) begin
        ...
        if(vif.monitor_cb.rlast) begin
          ...
          `uvm_info(get_full_name(), $sformatf("FULL AXI4 Read transaction collected: %s", tr.sprint()), UVM_LOW)
          tx_ap.write(tr.clone());
        end
        tr.obs_kind = AXI_R_CH;
        `uvm_info(get_full_name(), $sformatf("R transaction collected: %s", tr.sprint()), UVM_MEDIUM)
        r_ap.write(tr.clone());
      end else begin
