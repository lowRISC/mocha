...
module tb;
  ...
  `ASSERT(JustOneKmacResponse_A, kmac_data_in.done |=> always !kmac_data_in.done,
          clk, rst_n)
  ...
  `ASSERT(KmacNotOutAfterIn_A, kmac_data_in.done |=> always !kmac_data_out.valid,
          clk, rst_n)
  always_comb begin
    if (!rst_n) kmac_digest_handshake_occured = 0;
    else if (kmac_data_in.done) kmac_digest_handshake_occured = 1;
  end
