...
// A sequence item that represents the request that should be made to send a single AXI write with
...
function axi_fixed_write_req_item::new(string name = "");
  super.new(name);
  m_write_data_item = axi_write_data_item::type_id::create("m_write_data_item");
endfunction

function void axi_fixed_write_req_item::do_print(uvm_printer printer);
  ...
  printer.print_object("m_write_data_item", m_write_data_item);
endfunction

function void axi_fixed_write_req_item::do_copy(uvm_object rhs);
  axi_fixed_write_req_item rhs_;
  ...
  this.m_write_data_item.copy(rhs_.m_write_data_item);
endfunction

function bit axi_fixed_write_req_item::do_compare(uvm_object rhs, uvm_comparer comparer);
  axi_fixed_write_req_item rhs_;

  // These items are only equivalent if rhs is actually an axi_fixed_write_req_item.
  if (rhs == null || !$cast(rhs_, rhs)) begin
    comparer.print_msg("RHS is null or is not an axi_fixed_write_req_item.");
    return 0;
  end
  ...
  return (super.do_compare(rhs, comparer) &
          ...
          comparer.compare_object("m_write_data_item", m_write_data_item, rhs_.m_write_data_item));
endfunction
