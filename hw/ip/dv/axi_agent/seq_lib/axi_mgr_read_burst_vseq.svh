...
// To use it:
//
//   - Create it, then call set_sequencers() and set_read_response_router(). Both are required:
//      body() fatals if either is missing.
//   - Choose the request. m_ar_req is rand, so randomising this sequence picks a legal burst;
//      alternatively set m_ar_req's fields directly for a directed one. The burst length comes
//      from m_ar_req.m_len, so the sequence expects m_len + 1 beats.
...
class axi_mgr_read_burst_vseq extends uvm_sequence#(uvm_sequence_item, axi_fixed_read_rsp_item);
  ...
  // to m_read_beats. Returns early if a reset makes the router hand back a null item.
