...
//
//        if (!vseq.randomize() with { m_ar_req.m_addr == 'h1234;
//                                     m_ar_req.m_len  == 3; }) ...
//
...
//   - m_read_data holds the beats that arrived, in beat order.
//
// If a reset happens part-way through, the sequence still finishes and still populates rsp, but
// m_read_data is short: it only holds the beats that had already arrived, possibly none at all. A
// caller should check m_read_data.size() rather than assuming m_len + 1 beats came back.
...
task axi_mgr_read_burst_vseq::body();
  ...
  // Construct the read request sequence (AR). Note that ar_seq is not randomised. Instead, we set
  // its ar_seq.m_req to our m_ar_req variable. This variable was randomised as part of this
  // sequence.
...
task axi_mgr_read_burst_vseq::collect_read_beats(int unsigned n_beats);
  for (int unsigned i = 0; i < n_beats; i++) begin
    ...
    if (base_item == null) break;  // We have seen a reset
