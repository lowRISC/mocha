...
class axi_mgr_write_response_seq extends uvm_sequence #(axi_response_accept_item,
  ...
  // Copy the m_use_fixed_*/m_fixed_* values above, so that a configured sequence can be used as a
...
function void axi_mgr_write_response_seq::do_copy(uvm_object rhs);
  axi_mgr_write_response_seq rhs_;
