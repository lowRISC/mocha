...
//   - Fill m_data_items with one write data item per beat, in order. This is required: body()
//      fatals if it is empty, and AWLEN always follows it (size - 1) however the request was
//      chosen. The beats are sent verbatim except WLAST, which is forced on the final beat.
//   - Choose the rest of the request. m_aw_req is rand, so randomising this sequence picks legal
//      AW fields; alternatively set m_aw_req's fields directly for a directed burst. Populate
//      m_data_items first either way, so len_matches_data_c can tie m_len to the beat count.
//   - Optionally configure m_b_accept to control bready timing (see axi_mgr_write_response_seq).
//      Its m_use_fixed_*/m_fixed_* pins are copied to the accept. m_b_accept is deliberately not
...
//
// However the request was chosen, body() re-checks it against the AXI legality constraints in
...
task axi_mgr_write_burst_vseq::body();
  ...
  // Construct the write request sequence (AW). AWLEN always follows the supplied W beats, so fix
  // m_len before handing m_aw_req over to be sent verbatim. (In random mode len_matches_data_c
  // already ties m_len to the beat count; this assignment also covers the directed path, where the
  // constraint does not run.)
